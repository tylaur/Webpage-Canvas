...
function getRelativeCoords(mouse, state) {
     return {
...
function onMouseDown(mouse, state) {
     state.drawing = true;
     state.currentPath = [getRelativeCoords(mouse, state)]; // Reset on mouse down
...
function onMouseMove(mouse, state) {
     ...
     state.currentPath.push(getRelativeCoords(mouse, state));
     state.paths.push(state.currentPath); // Adding to it while mouse is moving
